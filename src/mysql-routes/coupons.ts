...
router.get('/', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const [coupons] = await pool.execute('SELECT * FROM coupons ORDER BY createdAt DESC');
    res.json(coupons);
  } catch (err: any) { 
...
router.put('/:id', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const [existingRows] = await pool.execute('SELECT * FROM coupons WHERE id = ?', [req.params.id]);
    ...
    const [updatedRows] = await pool.execute('SELECT * FROM coupons WHERE id = ?', [req.params.id]);
