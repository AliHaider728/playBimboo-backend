...
router.get('/', async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM categories WHERE status != "inactive" ORDER BY displayOrder ASC, name ASC');
...
router.get('/admin/all', authenticateToken, requireSuperAdmin, async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM categories ORDER BY displayOrder ASC, name ASC');
    res.json(buildTree(rows as any[]));
  } catch (error) {
    res.status(500).json({ error: 'Could not load categories' });
  }
...
router.post('/', authenticateToken, requireSuperAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const [newCat] = await pool.execute('SELECT * FROM categories WHERE id = ?', [id]);
...
router.put('/:id', authenticateToken, requireSuperAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const [existing] = await pool.execute('SELECT * FROM categories WHERE id = ?', [req.params.id]);
